...
Este projeto, pertencente à disciplina de Web2 com framework da UTFPR Guarapuava, tem como objetivo implementar progressivamente e de forma didática uma aplicação web. Esta aplicação é inspirada em um website que facilita a troca de trabalho de mergulho por hospedagem, mas com um diferencial: a segmentação em um site específico para mergulhadores profissionais e certificados que desejam viajar pelo mundo, oferecendo seus serviços em troca de estadia.

O frontend da aplicação foi desenvolvido utilizando HTML, CSS com o framework Tailwind, e JavaScript. O backend foi simulado através da implementação de uma API Fake, utilizando o JSON Server.
